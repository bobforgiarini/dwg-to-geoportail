...
Der Umschalter im Kopfbereich verwendet die Browser-History. Die lokal ausgewählte `File`-Referenz und ihr Ladeprofil bleiben beim Wechsel zwischen beiden Routen in einer gemeinsamen React-Sitzung erhalten; der Ziel-Viewer verarbeitet die Datei kontrolliert neu. Die Datei geht bei einem Neuladen oder Schließen der Seite verloren. Die MLightCAD-Seite und ihre Bibliotheken werden erst beim Aufruf von `/mlightcad` geladen.
...
Weitere Informationen: [Architektur](docs/architecture.md), [Netlify](docs/netlify.md), [Tests](docs/testing.md) und [Release 0.3.0](docs/releases/0.3.0.md).
